...
- **AC#4 — preCompact hook snapshots in-flight findings. ~~[IN SCOPE]~~ → DEFERRED to EPIC-011 (directing-party decision 2026-08-24).** The premise — a preCompact hook capturing an *in-flight findings buffer* to disk — assumes an artefact no shipped component produces (§ Report Pipeline step 1 collects fragments in the model's context). A real preCompact hook can only snapshot the event's `transcript_path`, and its value is realised solely in long, context-pressured runs — the runtime territory EPIC-011 owns, alongside the Stop hook — so with the 1M context window now GA this AC was descoped rather than built as a stub. **This AC no longer gates EPIC-010's completion**; it is re-homed under EPIC-011. FEAT-009's five-hook commitment is intact, re-allocated: SessionStart + PreToolUse + PostToolUse shipped, preCompact + Stop → EPIC-011.
...
| TASK-026 — preCompact Findings-Snapshot Hook | DRAFT — **DEFERRED to EPIC-011** | AC#4 descoped 2026-08-24 |
| TASK-027 — `/shannon-goal` Promotion-Authority Resync (corrective) | APPROVED | 2026-08-25 |
| TASK-028 — Per-Category Finding Counts in the Report Header *(sequenced before TASK-025)* | APPROVED | — |

**All six live child Tasks are APPROVED and archived** (TASK-022, 023, 024, 025, 027, 028). **TASK-026 (preCompact) was DEFERRED to EPIC-011 on 2026-08-24** — its AC#4 is descoped from this Epic (see § Activity Log). With TASK-027 approved on 2026-08-25, **EPIC-010 has no outstanding child work**: the corrective `/shannon-goal` resync closed the last known drift against `ux_guide.md` v1.3. Advancing the Epic to IMPLEMENTED / REVIEW / APPROVED is an Epic-gate decision reserved to the directing party and is the Epic's only remaining step. One inherited item travels with it — TASK-027 found `ux_guide.md` v1.3's own `/shannon-goal` example still printing the fixed legend its prose forbids, so the shipped skill's example deliberately diverges from the Guide's until `/document-review ux_guide.md` corrects it (routed to `docs/scratchpad.md`).
...
*Gate 3, 2026-08-25. Verified by the supervisor; **approved by the directing party** (Epic gates are surfaced, not self-approved, per the SIT-026 standing directive).*
...
- **2026-08-25** — IMPLEMENTING → IMPLEMENTED → REVIEW → **APPROVED** at Gate 3, approved by the directing party. Six live ACs verified (AC#4 descoped to EPIC-011 earlier the same day); AC#1 and AC#6 verified by live `/shannon-goal` invocation per their own stated method, the run's footer naming a Task/Epic authority mix distinct from the shipped example — the real test that it derives rather than copies. Two findings recorded, neither blocking: AC#1/AC#2 cite `ux_guide.md` v1.2 and are three versions stale (verification performed against v1.4, citation recorded superseded — the third instance in this Epic of binding to a version or example instead of the governing rule), and a deploy gap where `hooks/sessionstart.settings.json` had never reached the dogfood `.claude/` tree, closed at review. **EPIC-010 delivers FEAT-009's third phase**: `/shannon-goal` goal decomposition, the SessionStart health-summary hook, per-category report counts, presentation-customisation prose, and the zero-findings clean-run pattern.

- **2026-08-25** — **TASK-027 APPROVED** (supervisor authority, SIT-026 standing directive), closing the Epic's last outstanding child. The `/shannon-goal` promotion-authority footer in `shannon/skills/shannon-supervisor/SKILL.md` now names all four promotion targets with Spike's reservation condition and the `spike_gate_authority` field, and the fenced example is labelled a worked instance with the candidate-derived rule stated in prose — the durable half of the fix, since a hardcoded legend is what drifted in the first place. **Deviation upheld at Gate 3**: the example's footer was *not* ported from `ux_guide.md` v1.3, because the Guide's own example still prints the fixed legend its prose forbids; the rule was ported instead, leaving a deliberate Guide-vs-skill divergence routed to `/document-review ux_guide.md`. Also promoted the framework-general lesson *an illustrative example in a Guide is not a specification* to `docs/scratchpad.md` — two shipped defects (TASK-022, TASK-023) from one document. **Consequence**: EPIC-010 has no child work left; the Epic gate is the only remaining step.

- **2026-08-24** — **TASK-026 DEFERRED to EPIC-011; AC#4 descoped** (directing-party decision at TASK-026's Gate 1). The elaboration found the prepared premise unsound — the hook was specified to snapshot an *in-flight findings buffer* that no shipped component writes (§ Report Pipeline step 1 collects fragments in the model's context; `SKILL.md:153` names a "findings buffer" that does not exist). A bash preCompact hook's only concrete input is the event's `transcript_path`, so an honest hook snapshots the transcript or summarises it via `claude -p` — and its value lands only in long, context-pressured runs, the runtime territory EPIC-011 owns. With the 1M context window now GA, compaction mid-report is rare. Three options were surfaced (minimal transcript-snapshot / model-summary / descope); the directing party chose **descope**. TASK-026 stays DRAFT and unelaborated, its prepared drafts retained for EPIC-011 to reconceive; AC#4 is re-homed under EPIC-011 and no longer gates this Epic. **Consequence**: EPIC-010's implementation scope is delivered — only the corrective TASK-027 stands between it and IMPLEMENTED. **Doc-hygiene item routed to `scratchpad.md`**: `SKILL.md:153`'s "report findings buffer for preCompact" describes a non-existent artefact and should be softened to forward work when EPIC-011 reconceives the hook.
- **2026-08-24** — **TASK-025 APPROVED (Gate 3), archived.** Fifth child Task closed, self-approved on supervisor authority per the SIT-026 standing directive. The SessionStart hook ships as `scripts/sessionstart-summary.sh` + `hooks/sessionstart.settings.json`, deployed and merged into the live `settings.json` beside PreToolUse/PostToolUse; all eight ACs verified against ten fixture runs. Its real-world behaviour today is the legacy path — every report on disk predates the `**By category:**` line — naming the total and stating each report's age (4 and 50 days). Live auto-firing deferred to EPIC-011. EPIC-010 → 5 of 7; only TASK-026 (preCompact) remains before IMPLEMENTED.
- **2026-08-24** — **TASK-025 PLANNED (Gate 2, supervisor-authority self-approval).** Its prepared 2026-07-18 plan still spoke of "extracting the three counts"; it was rewritten to consume the shipped `**By category:**` line via the exact selection method TASK-028 verified (anchor on the line prefix, positional integer, non-ASCII middot, absent line ⇒ legacy path). Six steps against the EPIC-009 hook precedent, deliberately non-scope-gated with the Cadence State mute lever, unit-tested before commit, live auto-firing deferred to EPIC-011. The Drift-as-lead scratchpad question was **not** reopened — the two real reports are equivocal (07-05 Drift-led, 08-20 Gap-led), below the bar to churn a day-old Guide decision.
