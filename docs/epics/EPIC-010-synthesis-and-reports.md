...
| TASK-028 — Per-Category Finding Counts in the Report Header *(sequenced before TASK-025)* | IMPLEMENTED | — |

Three of seven child Tasks APPROVED and archived (two added on 2026-08-21 — one corrective, one split out of TASK-025's elaboration). Next Task in the planned order is **TASK-028**, then **TASK-025** (`/task-elaborate TASK-025`), which must follow TASK-023 (it reuses TASK-023's documented terse presentation shape) and ideally follows TASK-024 (clean-run shape settled before SessionStart injects it) — both satisfied.
