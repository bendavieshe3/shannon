...
- **2026-08-24** — **Inbound scope: the preCompact hook is re-homed here** by directing-party decision at TASK-026's Gate 1 (see EPIC-010 § Activity Log 2026-08-24). EPIC-010 specified the preCompact hook (its TASK-026) to snapshot an *in-flight findings buffer* that no shipped component produces; a real bash preCompact hook's only concrete input is the event's `transcript_path`, and its value lands only in long, context-pressured runs — this Epic's runtime territory. TASK-026 was **descoped from EPIC-010** and its prepared drafts retained for reuse. **At `/epic-elaborate EPIC-011`, fold the preCompact hook into this Epic** alongside the Stop hook (both are runtime-integration hooks), and reconceive TASK-026's open shape question — transcript-snapshot (mechanical, testable) versus model-summary via `claude -p`. The Overview's "Stop hook half — the final hook the cascade names" framing needs updating to name preCompact + Stop as this Epic's two hooks. FEAT-009's five-hook commitment is intact and re-allocated: SessionStart + PreToolUse + PostToolUse shipped under EPIC-009/010; preCompact + Stop land here.
