...
- **AC#2 — `SKILL.md` carries the self-identification line.** The supervisor's `SKILL.md` begins with the explicit self-identification line per the Skill Invocation Pattern at `technical_design.md` v1.2 § API Design → *Skill Invocation Pattern* — the line that confirms the skill activated on invocation. The exact phrasing follows the established Shannon pattern (e.g. `When invoked, start your response with: "Activating shannon-supervisor skill for [verb]."`). Derives from `technical_design.md` v1.2 § API Design → *Skill Invocation Pattern*.
