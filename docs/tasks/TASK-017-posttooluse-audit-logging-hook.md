...
*Ratified at Gate 2 (2026-06-27); reworked at Gate 1 to the author-in-`./shannon/` + snippet + scope-gating + unit-test shape, inheriting TASK-016's hook conventions.*
...
- **Added a `SHANNON_AUDIT_LOG` override env var** (minor, beyond the strict AC). The hook writes to `.claude/skills/shannon-supervisor/audit.log` by default (the AC#3-codified path) but honours `SHANNON_AUDIT_LOG` if set — a clean testability affordance that let AC#6's unit tests write to a temp log without polluting the real one (anticipated by AC#6's "temp log path" wording). The default path is unchanged; the override is purely additive.

### Gotchas

- **`settings.json` merge preserves the PreToolUse entry.** Merged via `jq '.hooks.PostToolUse = $post'` (set the PostToolUse key, leave PreToolUse alone) rather than a blunt overwrite. Verified post-merge: `.hooks.PreToolUse[0].hooks[0].command` still points at `pretool-writeguard.sh`, and the live `settings.json` now carries both hooks. The `mv` of the merged temp file printed a cosmetic `set owner/group … Operation not permitted` warning (couldn't copy ownership metadata across the boundary) but the content replaced correctly.
- **Runtime matcher vs hook logic for non-write tools.** The snippet's matcher is `Bash|Edit|Write`, so at runtime only those tools invoke the hook. The hook body itself, if handed a different tool (e.g. Read) directly in a test, will still log it (file_path branch) — the *matcher* is the runtime gate, not the script. Harmless, and noted so a reviewer running the script with off-matcher input understands the line count.
- **`jq` dependency (same as TASK-016) + runtime signal propagation (same deferral).** The hook uses `jq`, invoked only on the active path (after the `SHANNON_SUPERVISOR_SCOPE` check), so normal sessions pay nothing. Whether Claude Code propagates `SHANNON_SUPERVISOR_SCOPE` to the PostToolUse subprocess is TASK-018's runtime question — identical to TASK-016's; the unit suite proves the logic given the signal reaches it.
- **Audit log already gitignored; no new `.gitignore` line.** `git check-ignore .claude/skills/shannon-supervisor/audit.log` → `*.claude/`. The audit log does not yet exist (no real supervisor run has written it — first real entries are TASK-018's dogfood). The shippable targeted-ignore for projects that track `.claude/skills/` is routed to scratchpad M-6.
- **`SKILL.md` change is 1 ins / 1 del** — the existing one-line § Hook Integration PostToolUse bullet extended in place to name the audit-log path; additive in substance, nothing else in the section touched.

### Documents Updated

- None (mandated documents). TASK-017 ships the hook body + registration snippet + the additive `SKILL.md` PostToolUse-bullet amendment — net-new skill content under the tracked `./shannon/` source.
...
- **2026-06-27** — IMPLEMENTED: All 6 Steps executed; **8/8 unit tests pass**. **Step 1**: hook body authored at `./shannon/skills/shannon-supervisor/scripts/posttool-auditlog.sh` (`bash` + `jq`) — inert unless `SHANNON_SUPERVISOR_SCOPE` set; when active appends `ISO-8601<TAB>TOOL<TAB>args` (Bash → command; Write/Edit → file_path) to the audit log (default `.claude/skills/shannon-supervisor/audit.log`, override `SHANNON_AUDIT_LOG`); always exits 0 (observational). **Step 2**: registration snippet `./shannon/.../hooks/posttooluse.settings.json` (matcher `Bash|Edit|Write`); `SKILL.md` § Hook Integration PostToolUse bullet additively amended to name the audit-log path; PostToolUse entry **merged** into the live `./.claude/settings.json` via `jq '.hooks.PostToolUse = $post'` — **PreToolUse entry verified intact** (`pretool-writeguard.sh`). **Step 3**: body + snippet + `SKILL.md` deployed to `./.claude/`, hook executable, `diff` identical. **Step 4 — unit tests (AC#6)**: 8/8 pass — (a) signal absent → no log file, exit 0; (b) active + Write → exactly one line matching `^ISO-8601\tWrite\tdocs/supervisor/report.md$`; (c) append-only → second (Bash) invocation adds a second line, first preserved, Bash logs the command; (d) ISO-8601 timestamp shape verified. Temp log used and cleaned up. AC#2, AC#3, AC#6 closed. **Step 5**: six-target portability greps across body + snippet + `SKILL.md` → aggregate 0 (AC#5); `git check-ignore .claude/skills/shannon-supervisor/audit.log` → `*.claude/`, `.gitignore` unchanged (AC#4 — no redundant line). **Step 6**: `git diff` shows only tracked `./shannon/` changes (`M SKILL.md`, `A` snippet, `A` body) + bookkeeping; absence checks all 0 (no checkers/templates change, `pretool-writeguard.sh` untouched, `settings.json` gitignored not staged); SKILL.md 1 ins/1 del (PostToolUse bullet extended in place); live `settings.json` carries both PreToolUse + PostToolUse. AC#7 closed. **Both supervisor hooks now coexist in one `settings.json`** with the PreToolUse entry preserved. Ready for Gate 3 review via `/task-review TASK-017`.
- **2026-06-27** — IMPLEMENTING: Started execution of the 6-Step Plan (author body → snippet + SKILL.md amendment + settings merge → deploy → unit-test → portability/check-ignore → git diff scope).
- **2026-06-27** — PLANNED (Gate 2 approved). The Plan was reworked at Gate 1 to the author-in-`./shannon/` + snippet + scope-gating + unit-test shape (inheriting TASK-016's conventions); ratified here with only the stale header marker refreshed. The 6-Step Plan covers all seven ACs: Step 1 (author hook body — `SHANNON_SUPERVISOR_SCOPE` inert gate, then append `ISO-8601 TOOL args` to `audit.log`) closes AC#2/AC#3-logic; Step 2 (snippet + `SKILL.md` § Hook Integration PostToolUse-bullet amendment + live `settings.json` merge alongside the intact PreToolUse entry) closes AC#1/AC#3-codification; Step 3 (deploy body to `./.claude/`); Step 4 (unit-test inert / logs / append-only / format) closes AC#6; Step 5 (six-target portability greps + `git check-ignore` for the log) closes AC#5/AC#4; Step 6 (`git diff` scope-bounded verification, PreToolUse entry intact) closes AC#7. Dependencies confirmed: TASK-011/019/016 APPROVED. Risks reframed (inherited inert-by-default; settings-merge-must-not-clobber-PreToolUse; audit-log leak mitigated by the `*.claude/` ignore). **No new framework-general ambiguities** beyond the M-6 audit-log convention (updated at Gate 1) and the already-routed `SHANNON_SUPERVISOR_SCOPE` signal. Status: ELABORATED → PLANNED.
