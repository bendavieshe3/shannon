...
- **V6-stale text in technical_design.md** — Line ~92 reads "Concurrent creation is not protected — Shannon is a single-developer tool." Under V6, multiple agents may operate, so the "single-developer" framing is out of date. Address during technical_design.md Gate 1 review (paired with the cooperative-access note just added to technology_stack.md). Surfaced during technology_stack review as an adjacent finding #framework #technical-design #v6
