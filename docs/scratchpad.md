...
- **ux_guide.md inherited PENDING drift** — During technical_design Gate 1 review, the alignment subagent noticed ux_guide.md §Cascading Operations (lines 103-115) uses `PLAN-PENDING` as if canonical. Technical_design has now reconciled away from the `*-PENDING` model (option b — prepared drafts inside DRAFT children). UX_guide will need a coordinated update during its Gate 1 review to remove `PLAN-PENDING` references and describe the new "prepared draft" surface pattern instead #ux-guide #v6
